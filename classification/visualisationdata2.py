...
# 3=tête, 2=cou, 10=torse, 4=épaule G, 7=épaule D
# 1=bassin, 5=hanche G, 8=hanche D, 6=genou G, 9=genou D
# ============================================================
CHAINS = [
    [0, 1, 10, 2, 3],      # bassin → torse → cou → tête
    [10, 4],               # épaule gauche
    [10, 7, 8, 9],         # épaule droite → hanche D → genou D
    [1, 5, 6],             # hanche gauche → genou G
    [1, 8, 9],             # hanche droite → genou D
]
CHAIN_COLORS = ["#ffffff", "#4ecdc4", "#f7b731", "#a29bfe", "#fd79a8"]

# Bras/membres pour la trace
TRACE_JOINTS = [10, 4, 7]   # épaules + torse
...
def get_skeleton(X, seq_idx):
    """
    Keraal : (77, n_frames) = (11 joints x 7 dims, n_frames)
    → reshape (11, 7, n_frames) → transpose (n_frames, 11, 7)
    → garder x,y : (n_frames, 11, 2)
    """
    raw  = X[seq_idx]                        # (77, n_frames)
    n_fr = raw.shape[1]
    data = raw.reshape(11, 7, n_fr)          # (11, 7, n_frames)
    skel = data.transpose(2, 0, 1)           # (n_frames, 11, 7)
    return skel[:, :, :2]                    # (n_frames, 11, 2)
...
def generate_video(X, exercise_name, class_id, cluster_id,
    ...
    output_path = output_dir / f"{exercise_name}_class{class_id}_cluster{cluster_id}_rep{rep_number}_seq{rep_idx}.mp4"
...
for exercise_dir in exercise_dirs:
    ...
    if not train_file.exists():
        print(f"⚠️  Pas de données pour {exercise_dir.name}")
        continue
    ...
    if (exercise_dir / "fold0/run0").exists():
