...
#  CONFIG
# ══════════════════════════════════════════════════════════════════════
BASE      = Path(r"C:\Users\PC\rehab-motion-analysis\classification\UCDHE_clf_mc_MP\fold0\run0")
REP_CSV   = BASE / "class_a" / "representatives.csv"
EXERCISE  = "UCDHE_CLF_MC_MP — Military Press"
FPS       = 8
TRACE_STEP = 15
...
# ══════════════════════════════════════════════════════════════════════
#  SQUELETTE — 8 joints identifiés visuellement
#
#   Gauche          Centre        Droite
#   4  (main G)
#   5  (coude G)    3  (épaule G)
#                   3 ── 0        0  (épaule D)
#                                 2  (coude D)
#                                 1  (main D)
#   7  (hanche G)                 6  (hanche D)
#
#  Chaînes :
#   main G → coude G → épaule G → épaule D → coude D → main D
#   épaule G → hanche G
#   épaule D → hanche D
#   hanche G → hanche D
# ══════════════════════════════════════════════════════════════════════
CHAINS = [
    [3, 5, 4],       # épaule G → coude G → main G
    [0, 2, 1],       # épaule D → coude D → main D
    [3, 0],          # épaule G ── épaule D  (barre épaules)
    [3, 7],          # épaule G → hanche G
    [0, 6],          # épaule D → hanche D
    [7, 6],          # hanche G ── hanche D
]
CHAIN_COLORS = [
    "#4ecdc4",   # bras gauche — cyan
    "#f7b731",   # bras droit  — jaune
    "#ffffff",   # barre épaules — blanc
    "#a29bfe",   # torse gauche  — violet
    "#a29bfe",   # torse droit   — violet
    "#a29bfe",   # hanches       — violet
]
...
def get_skeleton(seq_idx):
    raw  = X[seq_idx]
    n_fr = raw.shape[1]
    skel = raw.reshape(8, 2, n_fr).transpose(2, 0, 1)
    ...
    for j in range(8):
        ...
        while fi < n_fr - 1:
            diff = np.abs(xy[fi, j] - xy[fi-1, j]).max()
            if diff > SEUIL:
                fj = fi + 1
                while fj < n_fr - 1:
                    if np.abs(xy[fj, j] - xy[fi-1, j]).max() <= SEUIL:
                        break
                    fj += 1
...
def pose_to_plot(pose):
    return pose[:, 0], -pose[:, 1]   # Y inversé coords pixel
...
def draw_frame(fi):
    ...
    ax_anim.scatter(x[USED_JOINTS], y[USED_JOINTS], color=JOINT_COL, s=60, zorder=5)
